...
class model2(generic_potential.generic_potential):
    ...
    def V1T_from_X(self, v1,v5, T, include_radiation=True):
        T = np.asanyarray(T, dtype=float)
        bosons = self.boson_massSq(v1,v5,T)
        fermions = self.fermion_massSq(v1,v5)
        y = self.V1T(bosons, fermions, T, include_radiation)
        return y

    def thermal_Pi_massSq(self, v1,v5, T):
    ...
    def Vtot(self, v1,v5, T, include_radiation=True):
        T = np.asanyarray(T, dtype=float)

        bosons = self.boson_massSq(v1,v5, T)
        fermions = self.fermion_massSq(v1,v5)
        thermal = self.thermal_Pi_massSq(v1,v5, T)
        y = self.V0(v1,v5)
...
# 注意这里修改了输入，将两个数组合并成一个 (2, 100) 的数组
z = m.Vtot(x1, y1,100)
...
plt.scatter(y_vals[min_index[1]], x_vals[min_index[0]], color='red', label=f'Minimum Point\n({y_vals[min_index[1]]}, {x_vals[min_index[0]]}, {min_value})')
