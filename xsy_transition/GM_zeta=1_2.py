...
class GM_Z2_15(generic_potential.generic_potential):
    ...
    def __init__(self, M1, M5, lam2, lam3, lam4):  # 重写generic_potential里的方法init
        self.Ndim = 2
    ...
    def check_BFB(self):  # Checking whether the BFB conditions are satisfied
        ...
        if self.lam3 < 0 and self.lam4 <= -self.lam3:
            return False
        if self.lam5 >= 0 and self.lam3 >= 0 and self.lam2 <= self.lam5 / 2.0 - 2.0 * np.sqrt(self.lam1 * (1 / 3 * self.lam3 + self.lam4)):
            return False
        if self.lam5 >= 0 and self.lam3 < 0 and self.lam2 <= self.lam5 * 5.0 / 12.0 - 2.0 * np.sqrt(self.lam1 * (1 / 2 * self.lam3 + self.lam4)):
            return False
        if self.lam5 < 0 and self.lam2 <= -self.lam5 / 4.0 - 2.0 * np.sqrt(self.lam1 * (1 / 2 * self.lam3 + self.lam4)):
            return False
        return True
...
def GM_EWPT_Scan(filename, n_required=70000):
    with open(filename, 'w') as f:
        n_obtained = 0
        while n_obtained < n_required:
            M1_rnd = generate_random_number(0, 800)
            M5_rnd = generate_random_number(0, 800)
...
if __name__ == "__main__":
    ...
    GM_EWPT_Scan("2zeta2GM_EWPT_Scan_%s.dat"%timestamp)
